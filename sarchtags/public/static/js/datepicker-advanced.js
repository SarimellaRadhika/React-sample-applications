$.fn.mydatepicker	= function(type)
{
	...
	//label set.
	var monthR	= ["January","February","March","April","May","June","July","August","September","October","November","December"];
	...
	var yearStart	= cDate.getFullYear()+20;
	var yearEnd		= 2017;
...
//var Y,m,d,H,mins;
	...
	var createDatepicker = function()
	{
		...
		d	        = datetime && dateCheck?Number(datetime[0].split("-")[0]):cDate.getDate();
        m		    = datetime && dateCheck?Number(datetime[0].split("-")[1])-1:cDate.getMonth();
		Y			= datetime && dateCheck?Number(datetime[0].split("-")[2]):cDate.getFullYear();
		...
		if(type == 1 || type == 3)
		{
			...
			var header_width_div= $("<div>").attr("id","header_width").appendTo(header);
			var dp_month		= $("<div>").attr("id","dp_date_month").appendTo(header_width_div);
			var dp_days			= $("<div>").attr("id","dp_date_days").appendTo(header_width_div);
			var dp_year			= $("<div>").attr("id","dp_date_year").appendTo(header_width_div);
		...
		{
			//Time picker part.
			var ampm	= $("<div>").attr("id","timings-header").appendTo(dp_body);	
				var ampmddd	=  	$( "#timings-header").html( "&nbsp;&nbsp;&nbsp;HH &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;MM" );
				var dp_time		= $("<div>").attr("id","dp_time").appendTo(dp_body);	
			var time_hour	= $("<div>").attr("id","dp_time_hour").appendTo(dp_time).attr("class","mousescroll");	
	...
	var setInput		= function()
	{
		if(type == 1 || type == 3)
		{
			...
			var d		= $("#dp_date_days").attr("data-days");
		     ...
		     var dateVal	= zeronate(d)+"-"+zeronate(m)+"-"+Y;
