...
$(document).ready(function () {
  ...
  $('.dynamic-overlay-slider-smoothscroller').scroll(function () {
    ...
    $(document).ready(function () {
      ...
      $('.new-tkn-input').on("focus", function () {
        $('.tkn-lable').addClass('active');
        $('.tokenfield-main-wrap').addClass('active');
      }).on("blur", function () {
        $('.tkn-lable').removeClass('active');
        $('.tokenfield-main-wrap').removeClass('active');
      });

      $(document).ready(function () {
        if ($('.tokenfield-main-wrap > .tokenfield > .new-scrollbar').children('.token').length >= 1) {
          $('.new-tkn-input').removeAttr('placeholder');
          $('.tkn-lable').addClass('active');
          $('.tokenfield-main-wrap').addClass('active');

          $('.new-tkn-input').on("focus blur", function (e) {
            e.stopImmediatePropagation();
          });
        }
      });

      /*---show/Hide channel search wrap--*/
      $('.show-search').click(function () {
        $('.channel-search-wrapper').show();
      });
      $('.cancel-act').click(function () {
        $('.channel-search-wrapper').hide();
      });