...
render()
{
    ...
    const reg1=new RegExp("^"+this.state.text+".*",'i');
    data1= data.filter((value)=>
    {return(value.name.match(reg1))})
    const reg2=new RegExp(".*"+this.state.text+"$",'i');
    data2= data.filter((value)=>
    {return(value.name.match(reg2))})
   /* const reg3=new RegExp("^this.state.text,'i');
    data3= data.filter((value)=>
    {return(value.name.match(reg3))})*/

    return(
        <div>
       SearchList::<input type="text" onChange={this.changeText}></input>
       <h4>Starts with {this.state.text} </h4>
       <ul>
       { data1.map((value,i)=>
    ...
    <hr/>
    <h4> Ends with {this.state.text}</h4>
        <ul>
       { data2.map((value,i)=>
